...
import { Tag, Users, Route, Ruler, Calendar, Clock, Home, Car, Phone } from 'lucide-react';
...
// Component to display a single trip detail
...
export default function TripCard({ booking }) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden border border-gray-200 cursor-pointer">
            ...
            {/* Header */}
            <div className={`p-5 flex justify-between items-start border-b-4 ${trip.status === 'Confirmed' ? 'border-green-500' : trip.status === 'Active' ? 'border-yellow-500' : 'border-red-500'}`}>
            ...
            <div className="p-5 space-y-4">
                <div className="flex items-center justify-between bg-indigo-50 p-3 rounded-lg border border-indigo-200">
                    <div className="flex flex-col">
                        <span className="text-xs font-semibold text-indigo-700 uppercase"> Booked</span>
                        <span className="text-2xl font-extrabold text-indigo-600 flex items-center gap-1">
                            {seatsBooked} <Users size={20} />
                        </span>
                    </div>
                    <div className="flex flex-col text-right">
                        <span className="text-xs font-semibold text-gray-500 uppercase">Total Seats</span>
                        <span className="text-lg font-bold text-gray-800">{trip.seats}</span>
                    </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <DetailItem icon={Route} label="Route" value={`${trip.source} to ${trip.destination}`} />
                    <DetailItem icon={Ruler} label="Distance" value={`${trip.kilometer} km`} />
                    <DetailItem icon={Calendar} label="Date" value={new Date(trip.date).toLocaleDateString()} />
            ...
            <div className="p-5 border-t border-gray-100 bg-gray-50">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    <div className="font-medium text-gray-800">
                        Vehicle No: <span className="font-bold text-indigo-600">{trip.vehicle}</span>
                    </div>
                    <div className="font-medium text-gray-800">
                        Provider Contact: <span className="font-bold">{trip.contact}</span>
                    </div>
                </div>
            </div>
        </div>
